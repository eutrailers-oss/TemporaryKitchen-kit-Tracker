...
export default function App() {
  ...
  async function saveJob(form,id,selected) {
    ...
    const del=await supabase.from('job_assets').delete().eq('job_id',jobId); if(del.error) throw del.error
    if(selected.length){const ins=await supabase.from('job_assets').insert(selected.map(asset_id=>({job_id:jobId,asset_id})));if(ins.error)throw ins.error}
  ...
  const pageProps={data,openJob,openAsset,openCustomer,openDamage,setPage,importAssets,refresh:loadAll}
  return <Layout page={page} setPage={setPage} user={session.user} signOut={()=>supabase.auth.signOut()}>
    ...
    {page==='dashboard'&&<Dashboard {...pageProps}/>} {page==='jobs'&&<Jobs {...pageProps}/>} {page==='assets'&&<Assets {...pageProps}/>} {page==='customers'&&<Customers {...pageProps}/>} {page==='damage'&&<Damage {...pageProps}/>} {page==='admin'&&<Admin {...pageProps}/>} 
