...
export default function Dashboard({ data, openJob, setPage }) {
  ...
  const physicallyOut = new Set(data.jobAssets.filter(x => outIds.has(x.job_id)).map(x => x.asset_id)).size
  ...
  return <>
    <header className="page-head"><div><h1>Dashboard</h1><p>Live overview of jobs and equipment.</p></div><button className="primary" onClick={()=>setPage('jobs')}>+ New job</button></header>
